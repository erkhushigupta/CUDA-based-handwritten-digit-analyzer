 ...
 * MSB first.
 */
static unsigned int mnist_bin_to_int(unsigned char *v)
{
    int i;
    unsigned int ret = 0;
...
// Convert 4 bytes from big-endian to int
unsigned int t_mnist_bin_to_int(unsigned char *bytes) {
    ...
    unsigned int *count)
{
    ...
    // fprintf(stdout, "line 65\n");
    fread(tmp, 1, 4, ifp);
    // fprintf(stdout, "tmp is %02x %02x %02x %02x\n", tmp[0], tmp[1], tmp[2], tmp[3]);
    // fprintf(stdout, "tminst %d\n", mnist_bin_to_int(tmp));
    if (mnist_bin_to_int(tmp) != 2051) {
        // fprintf(stdout, "line 69\n");
